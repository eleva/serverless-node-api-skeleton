...
/* Get Identity ID from lambda call (frontend) */
const getIdentityId = function (event) {
    // Added email for identify the developer.
    if (process.env.IS_OFFLINE && !!process.env.OFFLINE_IDENTITY_ID) {
        return process.env.OFFLINE_IDENTITY_ID;
    }

    return event?.requestContext?.identity?.cognitoIdentityId
        ?? parseBody(event?.body)?.requestContext?.identity?.cognitoIdentityId;
}
